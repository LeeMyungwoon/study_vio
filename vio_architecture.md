...
Bounded compute:
  ...
  더 작은 optimization budget B_opt
...
따라서 카메라 사이에서 같은 feature를 한 번도 연결하지 못해도, 각 camera residual이 동일한 `T_WB_k`를 제약하므로 이미 tight-coupled multi-camera VIO다. CCFT는 tight coupling의 성립 조건이 아니라 다음을 개선하는 보강 모듈이다.
...
| intrinsics, T_BC_i, camera-IMU delay, mid-exposure,          |
| optional rolling-shutter readout, IMU intrinsic compensation |
+-----------------------------+--------------------------------+
                              |
                              v
+--------------------------------------------------------------+
...
| fail: restore prior+state+landmarks+weights -> narrower ATLS |
+-----------------------------+--------------------------------+
                              |
             +----------------+----------------+
             |                                 |
...
Delta t_i          camera-IMU time offset
readout_i          rolling shutter readout time
...
bundle의 reference body time을 `t_k`라 하고 camera `i`의 실제 exposure center를 다음처럼 둔다.

\[
t_{k,i}=t_k+\Delta t_i+\Delta t^{\mathrm{mid}}_{k,i}.
\]

rolling shutter라면 row `r`의 관측 시각은:
...
t_{k,i}
+
\left(\frac{r}{H_i}-\frac{1}{2}\right)\tau_i,
\]

여기서 `tau_i`는 full-frame readout time이다. global shutter면 `tau_i=0`이다.

각 visual residual은 명목상 동일 bundle에 속하더라도 `t_{k,i}`의 body pose를 사용한다. 하드웨어 동기화가 있어도 exposure start/end와 sensor transport 지연은 0이라고 가정하지 않는다.
...
-> BCC
   ...
   fail -> stable snapshot restore -> narrower ATLS -> reoptimize
...
camera-IMU offset Delta t_i
exposure midpoint
rolling-shutter readout time (해당 시)
frame sequence / dropped-frame detection
```

MAVIS의 핵심 교훈은 “multi-camera 장치가 nominally synchronized”라는 사실만으로 시간 오차가 사라지지 않는다는 것이다. bandwidth 증가로 일부 frame이 누락되거나 camera별 exposure 시각이 달라질 수 있으므로, 남은 camera observation을 계속 사용하고 각 관측의 실제 시각으로 body pose를 보간해야 한다.
...
struct CameraFrame {
    CameraId camera_id;
    TimeNs sensor_timestamp;
    TimeNs exposure_midpoint;
...
camera timestamp uncertainty
rolling-shutter row-time uncertainty
```
...
source camera `i`의 landmark를 adjacent target camera `j`에 투영한다.
...
same bundle이면 body motion이 없어 보이지만 실제 exposure time이 다르면:
...
\mathbf T_{WB}(t_{k,i})
\mathbf T_{BC_i}
\frac{\pi_i^{-1}(\mathbf u_{k,i,l})}{\rho_l}
\right).
\]

이 식이 camera time offset / mid-exposure compensation을 포함한 최종식이다.
...
각 landmark의 robust pose information을 다음처럼 구성한다.
...
-> selected visual factors에 ATLS 적용
```
...
```text
1. weighted parallax
2. ATLS로 dynamic visual residual을 완전히 truncate
...
`w_o`는 `s_l`, `q_o`, ATLS auxiliary weight를 결합한다.
...
-> BCC
   ...
   fail:
     snapshot 전체 restore
     truncation range 축소
     suspect associations weight 감소 / provisional merge rollback
     re-optimization
```
...
small delta time offset_i with strong prior
```
...
landmark `l`의 host가 frame `h`, camera `c_h`이고 target observation이 frame `k`, camera `c_t`라면:
...
camera timestamps가 다르면 `T_WB_k` 대신 observation-time interpolated pose `T_WB(t_{k,c_t})`를 쓴다.

동일 landmark가 same bundle의 두 camera에서 관측되면 두 residual이 같은 body state와 같은 inverse depth에 연결된다.
...
E2. fail:
      snapshot restore
      suspicious association rollback
      truncation range 축소
      re-optimize
```
...
global place recognition:
  selected camera image or compressed descriptors

coverage policy:
  front/rear/left/right azimuth가 장기적으로 균형 있게 DB에 들어가도록 선택
```

pose node는 하나지만 descriptor views는 여러 개일 수 있다.
...
```text
NFR factors
+ loop reprojection / relative-pose factors
+ optional map priors
...
struct CameraCalibration {
    ...
    double exposure_midpoint_offset_sec;
    double readout_time_sec;
...
EstimationResult processBundle(const FrameBundle& bundle) {
    ...
    frontend.trackTemporal(bundle);
    projector.projectLocalMap(bundle);
    ccft.trackAdjacentCameras(bundle);
    landmarks.update(...);
    robust.updateConfidence(...);

    auto selected = budgeter.selectOptimizationSet(...);
    auto result = estimator.optimizeATLS(selected, ...);

    if (!robust.checkBiasConsistency(result).passed) {
        stable_state.restore(snapshot, ...);
        robust.narrowTruncationRange();
        landmarks.rollbackSuspectAssociations();
        result = estimator.optimizeATLS(...);
    }
    ...
    stable_state.commit(...);
    landmarks.rehostBeforeMarginalization(...);
    estimator.marginalize(...);
    nfr_exporter.exportStableFactors(...);
    return result;
}
```

실제 구현에서는 두 번째 BCC 실패 시 무한 reoptimization을 막는 bounded retry가 필요하다.
...
tracking thread가 분리되어도 residual은 동일한 `T_WB_k`와 동일 IMU state를 optimize한다.
...
각 camera observation은 `t_{k,i}`로 pose interpolation을 받는다. 따라서 partial synchronization과 rig bundle이 양립한다.
...
+ IMU interpolation to observation time
...
### Stage 6. MAVIS sensor handling and exact IMU

```text
IMU intrinsic compensation
exact SE_2(3) preintegration
camera delay / mid-exposure
...
Time model:
  ...
  mid-exposure compensation
  optional rolling-shutter row time
...
| dynamic robust loss | ATLS | large residual 영향 완전 truncate |
...
[ ] all camera timestamp offsets measured
