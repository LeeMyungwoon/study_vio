...
최종 target hardware는 global shutter camera이므로 같은 image의 모든 pixel은 동일한 `t_{k,i}`를 사용한다. 즉 rolling-shutter row-time residual은 기본 estimator에 넣지 않고, readout time은 `0`으로 둔다.
...
중요한 점은 `T_WB(t_{k,i})`가 독립 pose state가 아니라는 것이다. observation time이 bundle reference time과 다르면, surrounding window state와 IMU preintegration으로 만든 미분 가능한 interpolation / propagation operator를 사용한다.
...
visual residual의 Jacobian은 이 operator를 통해 연결된다.
...
\frac{\partial \mathbf T_{WB}(t_o)}{\partial \Delta t_i}
\right].
\]
...
exposure midpoint
global-shutter validation (readout time = 0)
...
하지만 최종 backend에서는 `rho_l`도 함께 optimize하므로 이 값을 그대로 pose information으로 쓰면 depth uncertainty를 무시해 과대평가할 수 있다. 따라서 candidate landmark별 whitened visual Jacobian을 pose block과 inverse-depth block으로 나눈다.
...
`T_WB(t_h)`와 `T_WB(t_o)`는 Section 1.3의 미분 가능한 interpolation / propagation operator로 window state에 연결된다. 이 pose들은 별도 independent pose node가 아니며, frozen propagated pose도 아니다.
...
struct CameraCalibration {
    ...
    double readout_time_sec;  // final global-shutter target: 0
...
[ ] global-shutter readout time is asserted as zero
